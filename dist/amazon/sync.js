import { RMAuthError, categoryNodeId, searchTransactions, setTransactionCategory, setTransactionNote, } from "../rm/client.js";
import { categorizeAmazonItem } from "./categorize.js";
import { fetchAmazonOrdersSince, isAmazonCharge } from "./email.js";
...
function matchOrder(txn, orders) {
    ...
    for (const order of orders) {
        if (order.orderTotal === null)
            continue;
        if (Math.abs(order.orderTotal - dollars) >= AMOUNT_TOLERANCE)
            continue;
...
export function sinceFromLookback(lookbackDays) {
 ...
 */
export async function syncAmazonSince(since, dryRun) {
    const summary = {
        since,
        dryRun,
        scanned: 0,
    ...
    const synced = readJson(SYNCED_FILE, {});
    for (const txn of txns) {
        ...
        try {
            ...
            if (err instanceof RMAuthError)
                throw err; // abort whole run - session dead
