...
import { type SyncSummary, sinceFromLookback, syncAmazonSince } from "./sync.js";
...
export function getSchedulerConfig(): SchedulerConfig {
  return { ...DEFAULTS, ...readJson<Partial<SchedulerConfig>>(CONFIG_FILE, {}) };
}
...
async function tick(): Promise<void> {
  ...
  if (sessionStatus().status !== "live") return; // wait for a live cookie (extension keeps it fresh)
  const dueMs = cfg.intervalHours * 3_600_000;
  if (cfg.lastRunAt && Date.now() - new Date(cfg.lastRunAt).getTime() < dueMs) return;
  ...
  try {
    ...
    console.log(
      `[amazon-scheduler] ran: ${summary.updated} updated, ${summary.skipped} skipped, ${summary.unmatched} unmatched`,
    );
  } catch (err) {
...
export function startAmazonScheduler(): void {
...
export async function runNow(dryRun: boolean, lookbackDays?: number): Promise<SyncSummary> {
  const cfg = getSchedulerConfig();
  const summary = await syncAmazonSince(sinceFromLookback(lookbackDays ?? cfg.lookbackDays), dryRun);
  if (!dryRun) setSchedulerConfig({ lastRunAt: new Date().toISOString(), lastSummary: summary });
  return summary;
}
